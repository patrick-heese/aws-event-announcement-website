...
2. Navigate to the `cloudformation` folder, build, and deploy:  
    ...
    cd cloudformation
    sam build
    sam deploy --guided --capabilities CAPABILITY_NAMED_IAM --tags Project=aws-event-announcement-website
    ```

> **Note**: Ensure the AWS CLI user (`aws configure`) or CloudFormation assumed role has sufficient permissions to manage **S3**, **SNS**, **API Gateway**, **Lambda**, and **IAM resources**.  
